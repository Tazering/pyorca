...
def is_point_in_obstacle(x, y, obstacles, padding=0.0):
    """
...
def collision_check_segment(p1, p2, obstacles, walls, padding=0.0, steps=5):
    """
...
def collision_check_hybrid(subpts, obstacles, walls, padding=0.0):
    """
...
def hybrid_astar(obstacles, walls, start_xy, goal_xy, step,
                 ...
                 goal_radius=0.2, max_iter=200000, padding=0.0):
    ...
    while frontier and expansions < max_iter:
        ...
        for steer_deg in steer_values:
            (nx, ny, nh), arc_pts = propagate_arc(cx, cy, ch, steer_deg, step, n_sub=2)
...
class HybridAStarPlanner:
    ...
    def timer_callback(self, event):
        ...
        path = hybrid_astar(obstacles, walls, start_xy, goal_xy, step,
                            ...
                            goal_radius=0.5, max_iter=40000, padding=0.0)
        if not path:
...
def main():
    rospy.init_node("hybrid_astar_planner_from_odometry_node_r1", anonymous=False)
